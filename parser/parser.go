...
/*
** Created a new instance of the Parser
 */
func New(l *lexer.Lexer) *Parser {
	...
	// Register parsing functions for prefix Operators
	p.prefixParseFns = make(map[token.TokenType]prefixParseFn)
	p.registerPrefix(token.IDENT, p.parseIdentifier)
	p.registerPrefix(token.INT, p.parseIntegerLiteral)
	p.registerPrefix(token.BANG, p.parsePrefixExpression)
	p.registerPrefix(token.MINUS, p.parsePrefixExpression)
	p.registerPrefix(token.MINUS_MINUS, p.parsePrefixAssignmentExpression)
...
/*
** Returns possible errors encountered during the parsing phase
 */
...
/*
** Parsers the progrann and returns the AST
** Errors encountered during parsing can be accessed with the Errors method
 */
...
func (p *Parser) parsePrefixAssignmentExpression() ast.Expression {
	stmt := &ast.AssignStatement{Token: p.curToken}

	p.nextToken()
	name := p.parseExpression(PREFIX)
	...
	stmt.Operator = p.prevToken.Literal
	stmt.Value = &ast.IntegerLiteral{Token: p.curToken, Value: 1}
